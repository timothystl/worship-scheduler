...
<!-- ══════════════════ LOGIN SCREEN ══════════════════ -->
<div id="login-screen" style="position:fixed;inset:0;z-index:9999;display:flex;
  align-items:center;justify-content:center;background:var(--blue-mist);">
  <div style="background:var(--white);border-radius:14px;padding:40px 36px;
    box-shadow:0 6px 32px rgba(0,0,0,.12);width:100%;max-width:360px;text-align:center;">
    <div style="width:52px;height:52px;border-radius:50%;background:var(--steel-anchor);
      display:flex;align-items:center;justify-content:center;margin:0 auto 14px;
      font-size:24px;color:var(--white);">✝</div>
    <h2 style="margin:0 0 4px;font-size:1.25rem;font-weight:700;color:var(--steel-anchor);font-family:var(--font-head);">Timothy Lutheran Church</h2>
    <p style="margin:0 0 28px;font-size:.82rem;color:var(--warm-gray);font-family:var(--font-body);">Worship Schedule Builder</p>

    <!-- Google Sign-In removed: GitHub Pages sets Cross-Origin-Opener-Policy: same-origin
         which breaks signInWithPopup (popup cleanup calls signOut() via COOP workaround),
         and signInWithRedirect is broken by Chrome's bounce-tracker storage partitioning.
         Use email + password below instead. -->

    <input type="email" id="fb-email" placeholder="Email" autocomplete="username"
      style="display:block;width:100%;box-sizing:border-box;padding:9px 12px;
    ...
    <button id="btn-fb-signin" style="width:100%;padding:9px;border:none;border-radius:8px;
      background:var(--steel-anchor);color:var(--white);font-size:.9rem;font-family:inherit;cursor:pointer;">
      Sign In
    </button>
    <button id="btn-fb-forgot" style="background:none;border:none;font-size:.78rem;
      color:var(--mid-steel);cursor:pointer;font-family:inherit;margin-top:6px;
      text-decoration:underline;display:block;width:100%;text-align:center;">
      Forgot password / set new password?
    </button>
    <div id="login-status" style="font-size:.8rem;min-height:20px;margin-top:10px;color:var(--danger-btn);"></div>
  </div>
</div>
...
function queueFbPush() {
  if (!fbAuth.currentUser) return;
  clearTimeout(_fbSyncTimer);
  _fbSyncTimer = setTimeout(fbPush, 1500);
}
...
function fbSignOut() {
  ...
  fbPush().finally(function() { fbAuth.signOut(); });
}
...
fbAuth.onAuthStateChanged(function(user) {
  ...
  var userDisplay = document.getElementById('fb-user-display');
  if (user) {
    // Enforce allowlist — applies to email/password and Google SSO alike
    var email   = (user.email || '').toLowerCase().trim();
    var allowed = ALLOWED_EMAILS.map(function(e) { return e.toLowerCase().trim(); });
    if (allowed.indexOf(email) === -1) {
      fbAuth.signOut().then(function() {
        updateLoginStatus('Access not authorized. Contact your administrator.', true);
      });
      return; // signOut re-triggers this callback with user=null, which restores login screen
    }
...
// ══════════════════════════════════════════════════════════════════
document.getElementById('btn-fb-signin').addEventListener('click', fbSignIn);
document.getElementById('btn-fb-forgot').addEventListener('click', fbForgotPassword);
document.getElementById('btn-fb-push').addEventListener('click', fbPush);
document.getElementById('btn-fb-pull').addEventListener('click', fbPull);
