...
- [ ] **G9** — Late-entry contributions: Breeze audit log filters by LOG DATE, so Dec contributions entered in Jan appear in the Jan audit log window but are currently excluded by our contribution-date filter. User wants these to be imported (they are logically part of the year they were logged in). Needs design decision: import them with their Dec contribution date into the 2026 sync, and handle potential double-import if user also syncs 2025. (noted 2026-04-18)
